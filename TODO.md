...
- [x] **Polarisation** — `quantum/3-1wave-behaviour-of-light/polarisation/`. Side-on beam with three always-present polariser slots (per-slot enable toggle + angle slider, drag-to-rotate disc on canvas). Discs drawn in perspective (foreshortened ellipse, face mostly toward source). Default view emphasises intensity drop-off; optional E-field arrow toggle reveals polarisation direction (unpolarised = random fixed angles per sample, polarised = single arrow along axis). Per-slot transmission factor `T` + bar; running `I_out / I₀` and final polarisation angle. Snap-to-15° toggle (default on) for slider and drag. Defaults to a single filter enabled — students discover multi-filter behaviour themselves.
